...
std::vector<MatchResult> LaneTracker::TrackWithMap(
  ...
  std::cout << "Candidate Landmark: " << candidate_lm_id.size() << std::endl;
  ...
  for (size_t i = 0; i < frame_observation.lane_observations.size(); ++i) {
    ...
    for (size_t j = 0; j < frame_observation.lane_observations.at(i).lane_points.size(); ++j) {
    ...
    // points_vector.clear();
    // points_vector.push_back(Eigen::Vector3d(8415.37,  227.418, -202.859));
    ...
    for (auto it_tree = lm_trees.begin(); it_tree != lm_trees.end();
      ...
      it_tree->second->QueryNearest(points_vector, matching_distance, matching_id);
  ...
  //   for (size_t j = 0; j < cost_vector.at(i).size(); ++j) {
  //     std::cout << cost_vector.at(i).at(j) << std::endl;
  //   }
  //   std::cout << "******\n";
  // }
