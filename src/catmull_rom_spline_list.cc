...
Eigen::MatrixXd CatmullRomSplineList::GetPoints(int num_points) const {
  ...
  // Calculate the total number of rows required for all matrices in points_list
  int total_rows = 0;
  for (const auto& points : points_list) {
    total_rows += points.rows();
  }

  // Create the all_points matrix with the calculated total number of rows
  Eigen::MatrixXd all_points(total_rows, ctrl_pts_.cols());

  // Fill all_points with the points from points_list
  int row_idx = 0;
  for (const auto& points : points_list) {
    all_points.block(row_idx, 0, points.rows(), points.cols()) = points;
    row_idx += points.rows();
  }
