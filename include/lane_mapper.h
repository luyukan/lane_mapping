...
class LaneMapper {
 ...
 private:
  ...
  void init_map(FrameObservation &frame_observation, const Odometry &pose);
  void track_with_last_frame(const FrameObservation &frame_observation,
                             const Odometry &pose);
  void track_with_map(FrameObservation &frame_observation,
                      const Odometry &pose);
  void smooth();
  void printInformation();

  void update_sw_status(const FrameObservation &frame_observation,
                        const Odometry &pose);
