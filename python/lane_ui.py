    ...
    def load_data(self):
        ...
        for topic, msg, timestamp in self.bag.read_messages():
            ...
            if gt_pose_msg is not None and lanes_gt_msg is not None and lanes_predict_msg is not None \
                ...
                pose_translation = gt_pose_wc[0:3, 3]
                pose_rotation = gt_pose_wc[0:3, 0:3]
                quat = R.from_matrix(pose_rotation).as_quat()
                timestamp = int(gt_pose_msg.header.stamp.to_sec() * 1e6)
                with open('/home/yukan/Downloads/OpenLane/lane3d_dataset/pose/' + str(timestamp) + '.txt', 'w') as f:
                    f.write(str(pose_translation[0]) + ' ' + str(pose_translation[1]) + ' ' + str(pose_translation[2]) + ' '
                            + str(quat[0]) + ' ' + str(quat[1]) + ' ' + str(quat[2]) + ' '+ str(quat[3]) + '\n')
                    
                with open('/home/yukan/Downloads/OpenLane/lane3d_dataset/lanes_gt/' + str(timestamp) + '.txt', 'w') as f:
                     for lane_id in range(lanes_gt_msg.num_lanes):
                        lane = lanes_gt_msg.lane_list[lane_id]
                        f.write(str(lane.category) + ' ') 
                        for lane_point_id in range(lane.num_points):
                          lane_point = lane.lane[lane_point_id]
                          f.write(str(lane_point.x) + ' ' + str(lane_point.y) + ' ' + str(lane_point.z) + ' ' + str(lane_point.visibility))
                          if lane_point_id == lane.num_points - 1:
                              f.write('\n')
                          else: 
                            f.write(' ')
                with open('/home/yukan/Downloads/OpenLane/lane3d_dataset/lanes_predict/' + str(timestamp) + '.txt', 'w') as f:
                     for lane_id in range(lanes_predict_msg.num_lanes):
                        lane = lanes_predict_msg.lane_list[lane_id]
                        f.write(str(lane.category) + ' ') 
                        for lane_point_id in range(lane.num_points):
                          lane_point = lane.lane[lane_point_id]
                          f.write(str(lane_point.x) + ' ' + str(lane_point.y) + ' ' + str(lane_point.z) + ' ' + str(lane_point.visibility))
