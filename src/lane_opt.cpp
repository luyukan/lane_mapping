...
LaneOptimizer::LaneOptimizer(const std::string& bagFile)
    : LaneTracker(bagFile) {
  // 初始化参数
...
void LaneOptimizer::add_keyframe() {
  if (use_isam) {
...
gtsam::SharedNoiseModel LaneOptimizer::get_pt_noise_model(
    double noise, bool huber = false, int dim = 3, double huber_thresh = 1.0) {
...
bool LaneOptimizer::set_gtsam_symbol(int lane_id, int ctrl_pt_id, Node* node) {
...
void LaneOptimizer::create_new_lane() {
...
gtsam::NonlinearFactor::shared_ptr LaneOptimizer::lane_factor(
...
gtsam::Vector LaneOptimizer::error_catmull_rom(
...
void LaneOptimizer::update_key_status(
...
void LaneOptimizer::add_chordal_factor(
  ...
  for (const auto& key : constrained_keys) {
    ...
    if (adj_key != 0 &&
        std::find(constrained_keys.begin(), constrained_keys.end(), adj_key) !=
            constrained_keys.end()) {
...
void LaneOptimizer::add_ctrl_factor(
    std::unordered_map<gtsam::Key, std::vector<int>>& key_status const
        gtsam::Vector3& ctrl_noise) {
...
void LaneOptimizer::buildGraph() {
    ...
    for (size_t i = 0; i < frame.get_lane_features().size(); ++i) {
      ...
      for (size_t j = 0; j < lf.get_xyzs().size(); ++j) {
        ...
        if (use_isam) {
          ...
          int interval_id =
              std::clamp(static_cast<int>(u * cfg.lane_mapping.lane_sample_num),
                         0, cfg.lane_mapping.lane_sample_num - 1);
          if (lf.noise[j] <
  ...
  for (const int lm_id : (use_isam ? std::vector<int>{} : lm_in_window)) {
    for (const auto& ctrl_pt_id : lane_grid[lm_id]) {
      for (int interval_id = 0; interval_id < cfg.lane_mapping.lane_sample_num;
           ++interval_id) {
...
void LaneOptimizer::slide_window() {
  if (use_isam) {
    return;
  }
...
bool LaneOptimizer::update_pose() {
  ...
  for (int i = 0; i < 1; ++i) {
    ...
    if (cur_frame->frame_id == 0) {
      graph.add(PriorFactorPose3(X(cur_frame->frame_id), Pose3(cur_frame->T_wc),
                                 odo_noise_model));
      ...
      initial_estimate.insert(X(prev_frame->frame_id), Pose3(prev_frame->T_wc));
    ...
    if (cfg.pose_update.reproject && !directions.empty()) {
      ...
      Vector3 degeneracy_d = std::accumulate(directions.begin(),
                                             directions.end(), Vector3::Zero());
...
void LaneOptimizer::optimization() {
  if (use_isam) {
